...
def m_1d2d_01_reg(input_shape):
    ...
    X = Dropout(0.3)(X)
    X = Flatten()(X)
    X = Dense(150, activation='relu', kernel_regularizer=regularizers.l2(0.001), name='fc')(X)
    X = Dropout(0.2)(X)
    X = Dense(7, activation='softmax', kernel_regularizer=regularizers.l2(0.001), name='softmax')(X)

    model = Model(inputs=X_input, outputs=X, name='m_1d2d_01_reg')    
